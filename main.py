...
from tinydb import TinyDB, Query  # लोकल डेटाबेस v4.8.0 के अनुकूल
...
# Local Database Setup (TinyDB v4.8.0 Compliant)
...
print("✅ लोकल TinyDB v4.8.0 डेटाबेस सफलतापूर्वक एक्टिव हो चुका है!")
...
# ⏱️ 🔄 रीस्टार्ट-प्रूफ ऑटो-डिलीट बैकग्राउंड टास्क
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        if results:
            ...
            # ⏰ भारतीय समय (IST UTC+5:30) बिना किसी ZoneInfo डिपेंडेंसी के
...
async def get_link_manually(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ADMIN_ID not in backup_queues or not backup_queues[ADMIN_ID]:
        await update.message.reply_text("❌ अभी बैकअप में कोई प्रोसेस की हुई फाइल नहीं है।")
        return
...
async def process_batch_queue(user_id, context: ContextTypes.DEFAULT_TYPE, message):
    ...
    if saved_files_meta:
        ...
        bot_username = (await context.bot.get_me()).username
        share_link = f"https://t.me/{bot_username}?start={batch_key}"
        await status_msg.edit_text(f"✅ सभी फाइल्स सुरक्षित स्टोर हो गई hain!\n\n🔗 आपका बैच लिंक:\n{share_link}", parse_mode="Markdown")
    else:
...
def main():
    ...
    print("🤖 Bot is successfully configured for python-telegram-bot v20.8!")
    app.run_polling()
